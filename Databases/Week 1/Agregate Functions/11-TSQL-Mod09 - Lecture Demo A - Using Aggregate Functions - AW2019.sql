...
-- 11-TSQL-Mod09 - Lecture Demo A - Using Aggregate Functions - AW2019

-- Demo Task 1: Scenario - User Story
/* 
The AdventureWorks Sales Manager wishes to know:
the average unit price, minimum ordered quantity and the maximum 
unit price discount for each sales order



What are the tables and fields of interest? Select and view the data in desired tables. 
Then scale a TSQL solution for desired results - Using built-in Aggregate functions
*/

-- Step 1: 
-- Change to AdventureWorks database
-- Also works with: USE AdventureWorks2016; GO
...
-- Step 2b: It is always advisable to review the main data from the table(s)
SELECT  SalesOrderID, ProductID, UnitPrice, OrderQty, UnitPriceDiscount
FROM     Sales.SalesOrderDetail
-- 
-- Step 2a: average unit price, minimum order and maximum unit price 
...
-- What does that mean? You can only group unitprice, orderqty or unitpricediscount.
-- Note We may count salesorderid and productid seperatly.

-- next step demonstrates how to AVG/MIN/MAX

-- Step 2b: Select and execute the following query to show
-- This will succeed and return the AVG/MIN/MAX desired rows:
-- We can only group unitprice, orderqty or unitpricediscount.
