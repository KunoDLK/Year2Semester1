...
namespace ThAmCo.Events.API
{
    public class MenuController
    {
        public static async Task<List<Menu>> Get()
        {
            ...
            {
                var response = await client.GetAsync($"{BaseAPI.BaseCateringURL}/Menus");
        ...
        public static async Task<Menu> Get(int id)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                var response = await client.GetAsync($"{BaseAPI.BaseCateringURL}/Menus/{id}");
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
        ...
        public static async Task Delete(int id)
        {
            using (HttpClient client = new HttpClient())
            {
                var response = await client.DeleteAsync($"{BaseAPI.BaseCateringURL}/Menus/{id}");
        ...
        public static async Task Put(Menu menu)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var response = await client.PutAsync($"{BaseAPI.BaseCateringURL}/Menus/{menu.MenuId}", content);
            }
        }
        ...
        public static async Task Post(Menu menu)
        {
            ...
            {
                var content = new StringContent(JsonConvert.SerializeObject(menu), Encoding.UTF8, "application/json");
                var response = await client.PostAsync($"{BaseAPI.BaseCateringURL}/Menus", content);
            }
        }
